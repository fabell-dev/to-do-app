...
export default function Notes({ notes }: { notes: Promise<ApiResponse> }) {
  ...
  return (
    <ul className="bg-background-color grid gap-x-10 gap-y-20 mx-10 grid-cols-3 ">
      {allNotes.map((note, index) => (
        <li
          key={note._id}
          className=" border-2 border-amber-300 bg-blue-300 h-80 flex flex-col items-center rounded-2xl justify-center"
        >
          <h1 className="text-3xl my-5">{note.title}</h1>
          <h2 className="mx-5">{note.content}</h2>
          <div className="flex felx-col gap-x-10 my-5">
            <NoteForm
              mode="edit"
              note={{
                id: note._id,
...
const handleDelete = async (id: string) => {
  if (confirm("¿Estás seguro de que quieres eliminar esta nota?")) {
    ...
    if (result.success) {
      toast.success(result.message || "Nota eliminado");
    } else {
